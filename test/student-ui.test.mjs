...
// 2026-09-08 刪除搬到 /settings/，**這一條要守的事沒有變**：
// 從學員頁一定走得到那個地方。守的是「找得到」，不是「按鈕在這一頁」。
test("★ 學員頁上一定走得到刪除帳號的地方", () => {
  const h = studentHTML(full, "");
  assert.match(h, /href="\.\.\/settings\/"/, "沒有一條通往設定的路");
  assert.match(h, /刪除帳號/, "要說出去那裡可以做什麼，不然沒有人會點");
  assert.doesNotMatch(h, /data-act="ask-delete"/,
    "兩個地方各有一顆會刪帳號的按鈕，哪天其中一顆改了行為，另一顆不會跟著改");
});
