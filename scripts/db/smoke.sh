...
# 不是 Supabase 真的那幾個。RLS 也驗不到（腳本用超級使用者跑，RLS 不套用）。
# 這支驗的是：語法、相依順序、以及那幾支函式**自己**的判斷邏輯
# （誰按得動、連點會不會寄兩封、刪帳號會不會連帶刪乾淨）。
# 真的要驗 RLS 與寄信，只有在 Supabase 上照 docs/2026-09-08-目標9-上線前測試清單.md 走一次。
set -euo pipefail
cd "$(dirname "$0")/../.."
...
chkraise() { # chkraise 名稱 錯誤字串 SQL
  local got; got=$(psql -d "$DB" -X -A -t -q -c "do \$\$ begin $3 raise notice 'NOT_BLOCKED'; exception when others then raise notice '%', SQLERRM; end \$\$;" 2>&1)
...
for f in supabase/migrations/2026-09-08-students.sql \
         ...
         supabase/migrations/2026-09-13-public-team.sql ; do
